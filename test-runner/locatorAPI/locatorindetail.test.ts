...
test("Locator in detail",async({page})=>{
    ...
    await test.step("Change price",async()=>{
        const priceSelect = await page.locator("select.product_sort_container")
        await priceSelect?.selectOption({value:"lohi"})
        const firstProduct = await page.locator("#inventory_container div.inventory_item_label a div")
